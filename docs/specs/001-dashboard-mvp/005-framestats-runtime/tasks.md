...
1. 作業ツリーのルートへ移動する。前提は wails CLI と Xcode コマンドラインツールが入っていること(`CLAUDE.md` の「開発コマンド」と同じ)。依存の取得は `wails build` が `wails.json` の `frontend:install`(`npm install`)で回すため、別途は要らない。`npm install` は `package-lock.json` を書き換えうるので、終了後に `git status` を確認する。
...
4. Web インスペクタを開く。**`-devtools` ビルドでは右クリックのコンテキストメニューが出ない**(wails v2.15.0 は `debug` タグが無いとき `disableDefaultContextMenu` を立て、production のランタイムが `contextmenu` を `preventDefault` する)。開き方は次のいずれか。
   ...
   - **⌘⇧F12 で開くことを実機で確認した(2026-09-06)。** Safari のアタッチは試していない。wails v2.15.0 は `WKWebView.isInspectable` を設定しない(`developerExtrasEnabled` のみ)ため、macOS 13.3 以降では Safari の「開発」メニューに現れない可能性がある。⌘⇧F12 を使うこと。
...
10. 途中で読みたくなったら `nullops.frameReport()` を実行する。5 秒の周期を待たずにその時点の 1 行が返る(有効化した直後で `recordFrame` が 1 度も走っていなければ `[framestats] no samples`、`recordFrame` が走り始めていれば各パネルの `n` が積み上がった行が返る。`n=0` の行は有効化直後の 1 フレームぶんの窓でしか出ないので、見られなくても異常ではない)。無効化を確かめるには `nullops.disableFrameStats()` を実行し、次の 3 つを観測する(要件 2.5): (a) 直後に `[framestats] disabled` の 1 行が出る、(b) 以降 5 秒ごとの報告行が止まる、(c) `nullops.frameReport()` が `[framestats] disabled` を返す。測り直すときはそのまま `nullops.enableFrameStats()` を実行し、`n` が小さい値から積み直すことを見る(標本が捨てられた証拠)。
...
| 観点 | 判定 | 主な指摘 | 対応 |
| :-- | :-- | :-- | :-- |
| 仕様適合(requirements-conformance) | 1 体目 NO-GO。対応後に同観点を再投入し GO(残った指摘は Nit 5 件で、すべて本書へ反映した) | `state.json` に範囲縮小前の手書き `blocked` が残り、存在しない節「D. 原因の切り分け」を参照していた(Critical) | エンジンのスキーマに無いキーを取り除いた。経緯は本書の付録に一本化 |
...
| 規約と構造 | GO | `samples.shift()` が毎フレーム O(n)(観点外の FYI) | 見送り。004 からの据え置きで本 unit の変更行ではない。実測で計測器自身の負荷が疑われたら別 unit で扱う(下の「残った懸念」節) |
...
| 1.2 | `console.info` の到達点 3 箇所が、起動後に何も操作しなければいずれも到達しない(2 箇所はコンソールからの明示呼び出しでのみ到達) | 静的な読み合わせ。実機は B.5 | 仕様適合 |
...
配布ビルドで「B. 配布ビルドでの実測」を実施した。5 秒ごとの報告を 13 行取得し、計測の対象 5 パネル(`commit`・`depgraph`・`gauge`・`scatter`・`timeseries`)すべてで mean 16.7 ms・p95 18.0 ms だった(p95 の例外は `depgraph` の 1 行の 19.0 ms のみ)。max は 23〜63 ms の散発。

**`requestAnimationFrame` のループの共有は入れないと決めた。** 004 の受け入れ基準 12.6 が持ち越していた判断はこれで閉じる。

判定にあたり、B.9 が置いた「境目付近(18〜22 ms)は結論にしない」という帯は当てはめなかった。この帯は連続量を前提にしているが、フレーム間隔は表示更新の周期で量子化されるためである。mean の 16.7 ms は 60 Hz の更新間隔そのもの(1000 ÷ 60 = 16.67)であり、フレームを 1 枚落とせば約 33 ms、2 枚で約 50 ms へ飛ぶ。18 と 33 のあいだに連続的な値は出ない。したがって p95 = 18.0 ms は閾値に近くて危ういのではなく、周期にほぼ完全に乗っていて 95 パーセンタイルでフレーム落ちが無いことを示す。

生の観測値は依頼の記録に残した(本書には数値を写さない。B.11 の方針)。

### 残った懸念

- **観測系の影響。** Web インスペクタを接続した状態で測るため、計測値がその影響を含む。2026-09-06 の実測では p95 が 18.0 ms で、接続の影響は悪い側にしか働かないため据え置きの判断を覆さないと結論した。境目付近の値を扱う必要が再び出たら、spec.md §8.1 の案 E(Go 側へ報告を送る)を別 unit として検討する。
- **計測器自身の負荷。** `samples.shift()` は 600 要素の配列に対して毎フレーム O(n) で、004 からの据え置き。本 unit で配布ビルドでも有効化できるようになったため、実測で計測器自身の負荷が疑われたらリングバッファ化を別 unit で扱う。
- **最大化時の値。** 004 spec.md §9.2 の「最大化して繰り返す」を必須にしていない。2026-09-06 の実測は既定のウィンドウサイズだけで行った。将来パネルを増やす等で判断をやり直す場合は、最大化時の値も取ること。
- ~~**Web インスペクタの開き方は実機未確認。**~~ 解消した。⌘⇧F12 で開く(2026-09-06 に実機で確認)。

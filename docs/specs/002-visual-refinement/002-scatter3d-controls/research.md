...
- unit #1 完了後の計測(spec.md §3 前提 5 が出典として記録する。リポジトリ・PR #34 に別の記録は無い): 2026-09-06 に中継役(人間)がホストの `wails build -devtools` ビルドで `window.nullops.enableFrameStats()` により計測し、13 回の出力すべてで 5 パネルとも p95 = 18.0 ms・mean 16.7 ms。3D 散布図の描画面積は 469×438 から 589×661 へ約 1.9 倍に広がった。

...
| トーラス(torus) | 固定した中心・大半径・小半径のトーラス面上の点の集合。roadmap §1・§1.1 の「らせん」に対応する |
...
- **地形(`S = 0`)**: 点ごとに `(x, z)` を `[-0.98, 0.98]²` の一様乱数で固定する。高さは
  ...
  で、`yFloor = −0.95`、峰 5 個の `(px_k, pz_k)` は `[-0.7, 0.7]²`・`A_k` は `[0.6, 1.8]`・`σ_k` は `[0.18, 0.4]`・位相オフセット `θ_k` は `[0, 2π)` の一様乱数で固定する。峰の重なりで `h` が `yTop = 0.9` を超える場合は `yTop` で切る。色値 `C = (h − yFloor) / (yTop − yFloor)`。振幅の `sin` 項が「峰が呼吸する」動きを作る。
...
**受け入れ基準**:
9.1. 完了時に `wails build -devtools` のビルドを起動し `window.nullops.enableFrameStats()` で計測したとき(手順は凍結済み `docs/specs/001-dashboard-mvp/005-framestats-runtime/tasks.md` B 節。1440×900)、システムは計測対象 5 パネル(`commit`・`depgraph`・`gauge`・`scatter`・`timeseries`)のいずれについても、報告の最後の 6 行(30 秒ぶん)すべてで p95 が 20 ミリ秒以上である状態にしてはならない(集計規則は B 節 9 と同じ。1 行でも 20 ms 未満なら超過と見なさない)。(イベント)

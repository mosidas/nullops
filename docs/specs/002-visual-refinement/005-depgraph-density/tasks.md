...
- 最終検証パネル(requirements-conformance・security・test の 3 観点、本ターン)の指摘と対応:
  - **[修正済み・Critical] `groupNodesByHealth` の Map 使い回しが受け入れ基準 6.1 を退行させていた**: requirements-conformance・test の両観点が独立に検出。直前のタスク(上記「毎フレーム割り当て」修正)で `groups.length = 0` により配列は空にしていたが、**Map のキー自体は削除していなかった**ため、`drawNodes` はそのフレームに該当ノードが 0 個の健康状態に対しても `ctx.fillStyle` を代入していた(例: `warn` のノードが一度出現した後 0 個に戻ったフレームでは、存在する種類数が 1〜2 でも代入回数が 3 回のまま)。`newGraphSource` は全ノードを `ok` で開始し `warn`/`down` を確率的に往復するため、通常運転で到達する。`drawNodes` のループ先頭に `if (group.length === 0) continue;` を追加し、空グループへの `fillStyle` 代入を止めた(`groupNodesByHealth` 自体・`RenderScratch` の使い回し方針・描画順序・輪郭線描画は変更していない)。修正後、健康状態が 3→2→1 種類と減る系列で代入回数が種類数と一致することを `node` の再現スクリプトで確認し、`npx tsc --noEmit`・`npm run lint`・`npm test`(70 件成功)で再確認した。この記録により、上のタスク 1.1・1.2 の「構造上保証する実装にした」という記述は本注記で補正する(spec.md・tasks.md 本体の該当節は凍結の規律により書き換えない)。
  - **[対応済み・Medium→解消] `TestGraphSourceCoreEdgesAlwaysConnected` の検証範囲**: 前ターンで見送っていた指摘を本ターンで対応済み(`graphsource_test.go`、コミット `cb38d8a`)。`s.Next()` の実際の戻り値 `Edges` から基幹エッジの対をすべて拾い出し、欠落と連結性の両方を毎フレーム確認する形にした。test 観点のレビューは、連結性の追加判定が実行上恒真(欠落があれば直前の `t.Fatalf` で打ち切られるため)であり検出力は「実返り値からの存在照合」側が担っている、と `[Nit]` で指摘している。実害はなく元の抜け穴(パッケージ変数のみを見ていた点)は解消済みのため、この `[Nit]` への追加対応は見送る。
  - **[対応見送り・Nit] `selectOptionalEdges` の候補数不足時の panic 経路**: security 観点が、`n > len(candidates)` のとき `candidates[perm[i]]` が index out of range で panic する理論上の経路を指摘。`graphOptionalEdgeCount`(36)と候補数(595)は乱数に依存しない定数であり外部入力から到達不能。CLAUDE.md の「panic はプログラマの誤りに限る」には整合する経路であり、悪用可能性がないため対応を見送る。
  - Low/FYI 指摘(旧 spec の番号を指す陳腐化コメント、`graphClusterCount` を配列長で表現している点、角度再抽選の総回数が spec 手順と 1 回ずれる点、`selectOptionalEdges` の候補配列サイズ見積もり等)は機能的な欠陥ではないため対応を見送った。
  - 判定: 上記 Critical 1 件を本ターンで修正し、3 観点とも GO(security は当初から APPROVED、requirements-conformance・test は Critical 修正により再判定なしで解消と判断。理由: 修正内容が両観点の指摘と 1 対 1 で対応し、他の合否判定に影響する変更を伴わないため)。
